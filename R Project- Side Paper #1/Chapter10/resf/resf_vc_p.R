...
resf_vc_p		<- function( y, y0 = NULL, x = NULL, xconst = NULL, xgroup = NULL, s_id = NULL, meig, method = "ml", penalty = "bic", maxiter = 30, sizelimit = 2000, g_np = NULL ){
    ...
    if( is.null( x ) ){
    	result	<- resf_p( y = y, x = xconst, xgroup = xgroup, s_id = s_id, meig = meig )
    	b_par	<- result$b
    	sf_par	<- result$s
    	e_stat	<- result$e
    	r	<- result$r
    	...
    	Mo 	<- crossprod( X0 )
    	mo	<- crossprod( X0, y )
    	parVmax_sq <- sqrt( sd( y ) / sd( as.numeric(y) - X0 %*% ( solve( Mo ) %*% mo ) ) ) # y is a pseries object, it has to be converted to numeric vector so that the denominator is conformable. We'll see what else will be needed.
	...
	LL	<- NULL
    	...
    	while( ( obj > sd( y ) * 0.0001 ) & ( iter <= maxiter ) ){
    		...
    		for( par0_sel in 1:( nsv + ng ) ){
    			...
    			while( er_dum == TRUE ){
    				...
    				er_dum	<- ( "try-error" %in% class(try1) ) | ( "try-error" %in% class(try2))
    				if( er_dum == TRUE ){
    	...
    	pred		<- X[ , null_dum3 ] %*% b
    	resid		<- as.numeric(y) - pred # Again, y is a pseries vector and has to be converted to a regular numeric vector before this can be calculated.
    	SSE		<- sum( resid ^ 2 )
    	...
    	r		<- NULL
    	other		<- list( x_id = x_id, nxf = nxf, xf_id = xf_id, df = df, b_s = b_s, b_covs = b_covs, evSqrts = evSqrts, model = "resf_vc" )
    }
    return( list( b = b_par, s = sf_par, e = e_stat, b_vc = b_vc, bse_vc = bse_vc, t_vc = bt_vc, p_vc = bp_vc,
    		pred = pred, resid = resid, vc = vc, r = r, b_g = bpar_g, s_g = bg_par, other = other ) )
}